    ...
    def add_people(self, *people):
        for p in people:
            if not isinstance(p, People):
                raise TypeError ('Объект должен принадлежать к классу People')
...
class Dom:
    ...
    def display(self):
        ...
        cnt = 1
            # print(self)
        for fl in self.floors:
            people_on_floor = sum(len(f.people) for f in fl.flats)

            print(f'{fl} Количество квартир - {len(fl.flats)}',
                      f' Количество жителей - {people_on_floor}')
            for f in fl.flats:
                print(f'\t\t{f}')
                for p in f.people:
                    print(f'\t\t\t{cnt:2}. {p.name:{mx}} - {p.age}')
